...
namespace Gladiators
{
    ...
    class Node2
    {
        ...
        double kickTreshold = 0.7;

        public double realValue;

        public Node2(int rel)
        {
            this.relations = rel;
        ...
        {
            double kick = 0;
            if (Score < this.kickTreshold)
            {
                kick = 1;
            }
            for (int i = 0; i < weights.Length; i++)
            {
                this.adaptedVelocities[i] = weightVelocities[i]*dScore + kick * this.kickFactor*this.weights[i]*(((double)learningRate)/100.0);
            }
